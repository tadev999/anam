...
const List<_EveningEmotion> _eveningEmotions = [
  _EveningEmotion(id: 'burnout',      emoji: '🔥', label: 'Kiệt sức',         subtitle: 'Hôm nay tiêu tốn nhiều năng lượng', accentColor: ZenTheme.mistRed),
  _EveningEmotion(id: 'overthinking', emoji: '🌀', label: 'Nhiều suy nghĩ',   subtitle: 'Tâm trí vẫn còn đang chạy', accentColor: ZenTheme.inkBlue),
  _EveningEmotion(id: 'lonely',       emoji: '🌑', label: 'Cô đơn',           subtitle: 'Thiếu đi sự kết nối chân thành', accentColor: ZenTheme.softGray),
  _EveningEmotion(id: 'empty',        emoji: '🫧', label: 'Trống rỗng',       subtitle: 'Không có gì đặc biệt xảy ra', accentColor: ZenTheme.creamWhite),
  _EveningEmotion(id: 'peaceful',     emoji: '☀️', label: 'Bình yên',         subtitle: 'Ngày hôm nay đi qua nhẹ nhàng', accentColor: ZenTheme.sageGreen),
  _EveningEmotion(id: 'grateful',     emoji: '🌸', label: 'Biết ơn',          subtitle: 'Có điều gì đó đáng trân trọng', accentColor: ZenTheme.softGold),
];
...
class _EveningReflectionViewState extends State<EveningReflectionView> with TickerProviderStateMixin {
  ...
  void _setEmotion(String id) {
    ...
    if (id == 'burnout')           dbRepo.currentMindState = 'burnout_state';
    else if (id == 'overthinking') dbRepo.currentMindState = 'overthinking_state';
    else if (id == 'lonely')       dbRepo.currentMindState = 'lonely_state';
    else if (id == 'empty')        dbRepo.currentMindState = 'lonely_state';
    else if (id == 'peaceful')     dbRepo.currentMindState = 'peaceful_state';
    else if (id == 'grateful')     dbRepo.currentMindState = 'peaceful_state';
    else                            dbRepo.currentMindState = null;
  }
  ...
  void _submitEvening() {
    ...
    anchorBloc.add(CompleteEveningReflectionRequested(
      uid: uid,
              ...
              listener: (context, state) {
                ...
                if (state is AnchorLoadSuccess && !_morningAnchorLoaded) {
                  setState(() {
                    ...
                    if (state.anchor != null && state.anchor!.eveningCompleted) {
                      _currentStep = 3;
            ...
            return Expanded(
              child: Container(
                ...
                decoration: BoxDecoration(
                  color: active ? ZenTheme.inkBlue : ZenTheme.creamWhite.withOpacity(0.1),
            ...
            return Expanded(
              child: Text(labels[i],
                ...
                style: TextStyle(
                  ...
                  color: active ? ZenTheme.inkBlue.withOpacity(0.9) : ZenTheme.softGray.withOpacity(0.35),
  ...
  Widget _buildStepContent(TextTheme textTheme, bool isLoading) {
    switch (_currentStep) {
      case 0: return _buildEmotionStep(textTheme);
      case 1: return _buildIntentionReviewStep(textTheme);
      case 2: return _buildNoteStep(textTheme, isLoading);
      case 3:
  ...
  Widget _buildEmotionStep(TextTheme textTheme) {
    return Column(
      ...
      children: [
        ...
        const Center(child: Icon(Icons.nightlight_outlined, color: ZenTheme.inkBlue, size: 22)),
        const SizedBox(height: 16),
        Text(
          'Ngày hôm nay đã mang đến\ncho bạn cảm xúc gì?',
          textAlign: TextAlign.center,
          style: textTheme.titleLarge!.copyWith(
            fontFamily: 'Lora', fontWeight: FontWeight.normal, fontSize: 22, height: 1.4,
          ),
        ),
        ...
        Text(
          ...
          textAlign: TextAlign.center,
          style: textTheme.bodyMedium!.copyWith(fontStyle: FontStyle.italic, color: ZenTheme.softGray),
        ),
  ...
  Widget _buildEmotionCard(_EveningEmotion em) {
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: isSelected ? em.accentColor.withOpacity(0.12) : ZenTheme.creamWhite.withOpacity(0.05),
          borderRadius: BorderRadius.circular(18),
          border: Border.all(
            color: isSelected ? em.accentColor.withOpacity(0.5) : ZenTheme.creamWhite.withOpacity(0.08),
        ...
        child: Column(
          ...
          children: [
            ...
            Text(em.label, textAlign: TextAlign.center,
              style: TextStyle(
                color: isSelected ? em.accentColor : ZenTheme.softGray,
                fontSize: 10, fontWeight: isSelected ? FontWeight.bold : FontWeight.normal, height: 1.2,
              )),
          ],
        ),
      ),
  ...
  Widget _buildEmotionAffirmation(TextTheme textTheme) {
    ...
    final emotionKey = _selectedEmotionId == 'grateful' ? 'peaceful' : (_selectedEmotionId ?? 'peaceful');
    final affirmations = ZenConstants.emotionalAffirmations[emotionKey];
    if (affirmations == null || affirmations.isEmpty) return const SizedBox.shrink();
    ...
    return Container(
      ...
      child: Text(
        ...
        style: textTheme.bodyMedium!.copyWith(
          ...
          fontStyle: FontStyle.italic, height: 1.5, fontSize: 13,
        ),
      ),
    );
  }
        ...
        if (_morningIntention != null && _morningIntention!.isNotEmpty) ...[
          GlassContainer(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              const SizedBox(height: 10),
              Text('"$_morningIntention"',
                style: textTheme.bodyLarge!.copyWith(
                  color: ZenTheme.creamWhite, fontStyle: FontStyle.italic, fontSize: 17, height: 1.5,
                )),
            ]),
          ),
          const SizedBox(height: 24),
          Text('Bạn đã thực hiện điều này chưa?',
            textAlign: TextAlign.center,
            style: textTheme.titleLarge!.copyWith(fontSize: 17, color: ZenTheme.creamWhite)),
          const SizedBox(height: 16),
          Row(children: [
            Expanded(child: _buildReviewChoice(
              emoji: '✅', label: 'Đã làm được!',
              sublabel: 'Tốt lắm, bạn đã giữ lời với bản thân',
              color: ZenTheme.sageGreen, selected: _intentionAchieved == true,
              onTap: () => setState(() => _intentionAchieved = true),
            )),
            const SizedBox(width: 12),
            Expanded(child: _buildReviewChoice(
              emoji: '🔄', label: 'Chưa hoàn thành',
              sublabel: 'Không sao — ngày mai là cơ hội mới',
              color: ZenTheme.inkBlue, selected: _intentionAchieved == false,
  ...
  Widget _buildReviewChoice({
    required String emoji, required String label, required String sublabel,
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        padding: const EdgeInsets.all(14),
        decoration: BoxDecoration(
          color: selected ? color.withOpacity(0.12) : ZenTheme.creamWhite.withOpacity(0.04),
          borderRadius: BorderRadius.circular(16),
          border: Border.all(
            color: selected ? color.withOpacity(0.4) : ZenTheme.creamWhite.withOpacity(0.08),
        ...
        child: Column(children: [
          ...
          Text(label, textAlign: TextAlign.center,
            style: TextStyle(color: selected ? color : ZenTheme.softGray,
              fontSize: 12, fontWeight: selected ? FontWeight.bold : FontWeight.normal)),
          const SizedBox(height: 4),
          Text(sublabel, textAlign: TextAlign.center,
            style: TextStyle(color: ZenTheme.softGray.withOpacity(0.6), fontSize: 10)),
        ]),
      ),
    );
  }
    ...
    return Column(
      ...
      children: [
        ...
        Center(child: Text('Một điều bạn học hoặc cảm nhận được hôm nay',
          ...
          style: textTheme.bodyLarge!.copyWith(color: ZenTheme.softGray))),
        const SizedBox(height: 8),
        Center(child: Text('(Không cần hoàn hảo — một từ, một câu đều được)',
          textAlign: TextAlign.center,
          style: textTheme.bodyMedium!.copyWith(
            fontStyle: FontStyle.italic, color: ZenTheme.softGray.withOpacity(0.5), fontSize: 12))),
        const SizedBox(height: 28),
        GlassContainer(
          opacity: 0.05,
          child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            const Icon(Icons.nights_stay_outlined, color: ZenTheme.inkBlue, size: 26),
            const SizedBox(height: 12),
            TextField(
              ...
              decoration: InputDecoration(
                hintText: "Ví dụ: \"Hôm nay tôi học được rằng...\" hoặc chỉ \"Cần nghỉ ngơi nhiều hơn\"",
                hintStyle: TextStyle(color: ZenTheme.softGray.withOpacity(0.4), fontSize: 12),
        ...
        const SizedBox(height: 20),
        // Gợi ý nhanh
        const Text('Gợi ý:', style: TextStyle(color: ZenTheme.softGold, fontSize: 12)),
        const SizedBox(height: 8),
        Wrap(
          ...
          children: [
            ...
            onTap: () {
              ...
              _noteController.selection = TextSelection.fromPosition(TextPosition(offset: hint.length));
            },
            child: Container(
              ...
              child: Text(hint, style: TextStyle(color: ZenTheme.softGray.withOpacity(0.8), fontSize: 11)),
            ),
          )).toList(),
        ),
      ],
        ...
        AnimatedBuilder(
          ...
          builder: (context, _) => Transform.scale(
            ...
            child: Container(
              ...
              child: const Icon(Icons.nightlight_rounded, color: ZenTheme.inkBlue, size: 40),
            ),
          ),
        ),
        const SizedBox(height: 32),
        ...
        if (selectedEmotion != null)
          Container(
            ...
            decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(16),
              border: Border.all(color: selectedEmotion.accentColor.withOpacity(0.15)),
            ),
            child: Row(children: [
              Text(selectedEmotion.emoji, style: const TextStyle(fontSize: 28)),
              const SizedBox(width: 14),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text('Cảm xúc hôm nay: ${selectedEmotion.label}',
                  style: TextStyle(color: selectedEmotion.accentColor, fontWeight: FontWeight.w600)),
                const SizedBox(height: 4),
                Text(selectedEmotion.subtitle,
                  style: TextStyle(color: ZenTheme.softGray.withOpacity(0.7), fontSize: 12)),
              ])),
            ]),
          ),
  ...
  Widget _buildNavigationButtons(bool isLoading) {
    bool canProceed;
    switch (_currentStep) {
      case 0: canProceed = _selectedEmotionId != null; break;
      case 1: canProceed = _intentionAchieved != null || _morningIntention == null || _morningIntention!.isEmpty; break;
      case 2: canProceed = true; // Note là optional
      ...
      children: [
        if (_currentStep > 0)
          Expanded(
            child: Padding(
              ...
              child: ZenButton(
                text: 'Quay lại', isSecondary: true,
        ...
        Expanded(
          child: Opacity(
            ...
            child: ZenButton(
              text: _currentStep == 2 ? 'Hoàn thành buổi tối' : 'Tiếp tục',
              isLoading: isLoading,
